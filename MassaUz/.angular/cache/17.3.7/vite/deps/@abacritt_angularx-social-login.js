...
var _SocialAuthService = class _SocialAuthService {
   ...
   */
  constructor(config2, _ngZone, _injector) {
    this._ngZone = _ngZone;
    this._injector = _injector;
    ...
    if (config2 instanceof Promise) {
      config2.then((config3) => {
        this.initialize(config3);
      });
    } else {
  ...
  initialize(config2) {
    this.autoLogin = config2.autoLogin !== void 0 ? config2.autoLogin : false;
    const {
      onError = console.error
    } = config2;
    config2.providers.forEach((item) => {
    ...
    return new Promise((resolve, reject) => {
      this.loadScript(_MicrosoftLoginProvider.PROVIDER_ID, "https://alcdn.msauth.net/browser/2.13.1/js/msal-browser.min.js", () => {
        try {
          ...
          this._instance = new msal.PublicClientApplication(config2);
          resolve();
        } catch (e) {
          reject(e);
        }
